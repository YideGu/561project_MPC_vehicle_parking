%%This script is for Experiment 2
%Apply MPC on kinematic model, and get trajectory using dynamic model,
%without noise
clear;
clc;
%% parameters
...
plot(x_store_d(1, 1:length(x_store_d) - 10), x_store_d(2, 1:length(x_store_d) - 10),'r--');hold on; 
plot(Y_ref(1, 1: length(Y_ref) - 10), Y_ref(2, 1:length(Y_ref) - 10),'k');hold on;
legend('dynamic trajectory','reference')
xlabel('x')
ylabel('y')
