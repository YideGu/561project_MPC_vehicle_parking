...
[x,y,phi] = kinematic_model(6,[7.5 7.5 pi]',b,L,U_ref(1:2,1:601));
...
[x,y,phi] = kinematic_model(6,[7.5 7.5 pi]',b,L,U_ref(1:2,1:600));

Y_ref(1:3,2:601)=[x;y;phi];

U_ref(1:2,601:1201)=[ones(1,601) * 1; ones(1,601) * 0.85];
...
N_R=[1 1 0.01 0.01 0.0001 0.0001];
...
if Kalman_on==1
    hold on;
    plot(x_store_d(1, 1:length(x_store_d) - 10), x_store_d(2, 1:length(x_store_d) - 10),'b--');
plot(Y_ref(1, 1: length(Y_ref) - 10), Y_ref(2, 1:length(Y_ref) - 10),'b');
legend('Noise with fitler')
else
    hold on;
    plot(x_store_d(1, 1:length(x_store_d) - 10), x_store_d(2, 1:length(x_store_d) - 10),'r--'); 
plot(Y_ref(1, 1: length(Y_ref) - 10), Y_ref(2, 1:length(Y_ref) - 10),'b');
legend('Noise without fitler')
end
